...
def get_post_id():
    ...
    try:
        for url in urlList:
            ...
            post_id = soup.find('input', type="hidden")['value'].strip()
...
class AddCommentsThread(threading.Thread):
    ...
    def get_comment(self, judge):
        # 判断是否是第一次评论
        if judge == 1:
            filename = first_commentList_filename
        else:
            filename = commentList_filename

        # 读取csv文件里的评论
        with open(filename, 'r') as f:
            reader = csv.reader(f)
            comList = list(reader)
        comment = random.choice(comList)[0]
    ...
    def add_comment(self, post_id):
        comment_times1 = random.randint(5, 10) # 30分钟之内5~10条评论
        comment_times2 = random.randint(11, 60) # 24小时内20条评论
        ...
        while success_times <= comment_times2:
            try:
                ...
                requests.post(url=comment_url, headers=header, data=form_data)
                ...
                if success_times <= comment_times1:
                    ...
                    time.sleep(random.uniform(660, 3600))
                success_times += 1
                continue
